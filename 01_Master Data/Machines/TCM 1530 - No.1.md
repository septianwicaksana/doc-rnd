---
type: machine
machine_id: 100101
machine_name: TCM 1530 No.1
machine_type: Laser Cutting
manufacturer: TCM
model: 1530
machine_no: 01530
serial_no: TBD
category: Material Cutting
technology: Laser Cutting
country_of_origin: REP. RAKYAT CINA
year_acquired: 2021
year_manufacture: TBD
location: TBD
axis_config: TBD
bar_capacity: TBD
status: active
person_in_charge: Ahmad Jaelani
person_in_charge_2: Raditya Putra Wiratama
---

# TCM 1530 No.1

## Basic Information

| Field | Value |
| --- | --- |
| Machine ID | 100101 |
| Machine Name | TCM 1530 No.1 |
| Machine Type | Laser Cutting |
| Manufacturer | TCM |
| Model | 1530 |
| Serial No. | TBD |
| Country of Origin | REP. RAKYAT CINA |
| Year Acquired | 2021 |
| Person in Charge | Ahmad Jaelani |
| Person in Charge 2 | Raditya Putra Wiratama |
| Status | active |

## Capability

| Feature | Value |
| --- | --- |
| Axis Config | TBD |
| Bar Capacity | TBD mm |
| Max RPM | TBD |
| Max Feed | TBD |
| Turret Positions | TBD |

## Tooling System

TBD

## Tool Position Map

TBD

## Compatible Operations

TBD

## Product History

> Gunakan Dataview untuk menampilkan Job Card terkait mesin ini:

```dataview
TABLE rd_order_no, product, status, created_date
FROM "02_RnD Job Cards"
WHERE type = "rnd_job_card" AND machine = "TCM 1530 No.1"
SORT created_date DESC
```

## CNC Program History

TBD

## Common Issues

TBD

## Setup Notes

TBD

## Maintenance / Limitation Notes

TBD

## Related Job Cards

TBD

## Open Questions

| No. | Pertanyaan | Mengapa Penting | Status |
| ---: | --- | --- | --- |
| 1 | Axis configuration belum terdokumentasi | Dibutuhkan untuk kompatibilitas tool | Open |
| 2 | Bar capacity belum terdokumentasi | Dibutuhkan untuk menentukan ukuran material yang bisa diproses | Open |