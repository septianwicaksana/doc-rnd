---
type: machine
machine_id: 990101
machine_name: Provit W4L No.1
machine_type: Water Distiller
manufacturer: Provit
model: W4L
machine_no: 00W4L
serial_no: TBD
category: Aquades Provider
technology: Water Distiller
country_of_origin: Indonesia
year_acquired: 2023
year_manufacture: TBD
location: TBD
axis_config: TBD
bar_capacity: TBD
status: active
person_in_charge: Raditya Putra Wiratama
person_in_charge_2: Erlangga Pratama
---

# Provit W4L No.1

## Basic Information

| Field | Value |
| --- | --- |
| Machine ID | 990101 |
| Machine Name | Provit W4L No.1 |
| Machine Type | Water Distiller |
| Manufacturer | Provit |
| Model | W4L |
| Serial No. | TBD |
| Country of Origin | Indonesia |
| Year Acquired | 2023 |
| Person in Charge | Raditya Putra Wiratama |
| Person in Charge 2 | Erlangga Pratama |
| Status | active |

## Capability

| Feature | Value |
| --- | --- |
| Output Capacity | TBD L/h |
| Storage Capacity | TBD L |
| Water Quality Output | TBD |
| Power | TBD W |

## Tooling System

Tidak berlaku untuk tipe mesin ini.

## Tool Position Map

Tidak berlaku untuk tipe mesin ini.

## Compatible Operations

TBD

## Product History

> Gunakan Dataview untuk menampilkan Job Card terkait mesin ini:

```dataview
TABLE rd_order_no, product, status, created_date
FROM "02_RnD Job Cards"
WHERE type = "rnd_job_card" AND machine = "Provit W4L No.1"
SORT created_date DESC
```

## CNC Program History

Tidak berlaku untuk tipe mesin ini.

## Common Issues

TBD

## Setup Notes

TBD

## Maintenance / Limitation Notes

TBD

## Related Job Cards

TBD

## Open Questions

| No. | Pertanyaan | Mengapa Penting | Status |
| ---: | --- | --- | --- |
| 1 | Axis configuration belum terdokumentasi | Dibutuhkan untuk kompatibilitas tool | Open |
| 2 | Bar capacity belum terdokumentasi | Dibutuhkan untuk menentukan ukuran material yang bisa diproses | Open |