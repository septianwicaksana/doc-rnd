---
type: machine
machine_id: 200501
machine_name: Inovatec CB36A No.1
machine_type: Centrifugal Barrel Finishing
manufacturer: Inovatec
model: CB36A
machine_no: CB36A
serial_no: TBD
category: Debburing Polishing
technology: Centrifugal Barrel Finishing
country_of_origin: TBD
year_acquired: TBD
year_manufacture: TBD
location: TBD
axis_config: TBD
bar_capacity: TBD
status: active
person_in_charge: Fajar Sukatma
person_in_charge_2: Erlangga Pratama
---

# Inovatec CB36A No.1

## Basic Information

| Field | Value |
| --- | --- |
| Machine ID | 200501 |
| Machine Name | Inovatec CB36A No.1 |
| Machine Type | Centrifugal Barrel Finishing |
| Manufacturer | Inovatec |
| Model | CB36A |
| Serial No. | TBD |
| Country of Origin | TBD |
| Year Acquired | TBD |
| Person in Charge | Fajar Sukatma |
| Person in Charge 2 | Erlangga Pratama |
| Status | active |

## Capability

| Feature | Value |
| --- | --- |
| Barrel Capacity | TBD L |
| RPM Range | TBD RPM |
| Media Type | TBD |
| Compatible Operations | Deburring, Polishing, Finishing |
| Compatible Materials | Ti6Al4V ELI, SS316LVM |

## Tooling System

Tidak berlaku untuk tipe mesin ini.

## Tool Position Map

Tidak berlaku untuk tipe mesin ini.

## Compatible Operations

TBD

## Product History

> Gunakan Dataview untuk menampilkan Job Card terkait mesin ini:

```dataview
TABLE rd_order_no, product, status, created_date
FROM "02_RnD Job Cards"
WHERE type = "rnd_job_card" AND machine = "Inovatec CB36A No.1"
SORT created_date DESC
```

## CNC Program History

Tidak berlaku untuk tipe mesin ini.

## Common Issues

TBD

## Setup Notes

TBD

## Maintenance / Limitation Notes

TBD

## Related Job Cards

TBD

## Open Questions

| No. | Pertanyaan | Mengapa Penting | Status |
| ---: | --- | --- | --- |
| 1 | Axis configuration belum terdokumentasi | Dibutuhkan untuk kompatibilitas tool | Open |
| 2 | Bar capacity belum terdokumentasi | Dibutuhkan untuk menentukan ukuran material yang bisa diproses | Open |