---
type: machine
machine_id: 800101
machine_name: Epson TM-C3510 No.1
machine_type: Label Printing
manufacturer: Epson
model: TM-C3510
machine_no: 03510
serial_no: TBD
category: Labeling
technology: Label Printing
country_of_origin: Indonesia
year_acquired: 2022
year_manufacture: TBD
location: TBD
axis_config: TBD
bar_capacity: TBD
status: active
person_in_charge: Erlangga Pratama
person_in_charge_2: Raditya Putra Wiratama
---

# Epson TM-C3510 No.1

## Basic Information

| Field | Value |
| --- | --- |
| Machine ID | 800101 |
| Machine Name | Epson TM-C3510 No.1 |
| Machine Type | Label Printing |
| Manufacturer | Epson |
| Model | TM-C3510 |
| Serial No. | TBD |
| Country of Origin | Indonesia |
| Year Acquired | 2022 |
| Person in Charge | Erlangga Pratama |
| Person in Charge 2 | Raditya Putra Wiratama |
| Status | active |

## Capability

| Feature | Value |
| --- | --- |
| Print Width | TBD mm |
| Print Speed | TBD mm/s |
| Resolution | TBD dpi |
| Compatible Label Types | TBD |

## Tooling System

Tidak berlaku untuk tipe mesin ini.

## Tool Position Map

Tidak berlaku untuk tipe mesin ini.

## Compatible Operations

TBD

## Product History

> Gunakan Dataview untuk menampilkan Job Card terkait mesin ini:

```dataview
TABLE rd_order_no, product, status, created_date
FROM "02_RnD Job Cards"
WHERE type = "rnd_job_card" AND machine = "Epson TM-C3510 No.1"
SORT created_date DESC
```

## CNC Program History

Tidak berlaku untuk tipe mesin ini.

## Common Issues

TBD

## Setup Notes

TBD

## Maintenance / Limitation Notes

TBD

## Related Job Cards

TBD

## Open Questions

| No. | Pertanyaan | Mengapa Penting | Status |
| ---: | --- | --- | --- |
| 1 | Axis configuration belum terdokumentasi | Dibutuhkan untuk kompatibilitas tool | Open |
| 2 | Bar capacity belum terdokumentasi | Dibutuhkan untuk menentukan ukuran material yang bisa diproses | Open |